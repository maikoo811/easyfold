...
- 2026-05-30: GitHub Free plan branch protection is **Public-repo only** — Private repos on the free tier can't have any protection rules. If you want secure-by-default-from-second-zero on a flip, you either pay for Pro or accept a ~30 s gap between the Private→Public flip and the `gh api PUT branches/{branch}/protection` call. For a fresh repo nobody knows about yet, the window is harmless. Worth knowing for repos with pre-flip notoriety. (From Task 5.2.)
- 2026-05-30: A multi-day Public-flip window is **high-quality forcing-function input** — keep reviewing during it. Task 5.2 was a UI-only flip nominally taking 1 minute, but waiting on the user's confirmation gave three rounds of external (Clearline) review time to surface real medium-priority items (license clarity table, input caps, bearer-secret UI banner, A3M format validation, opt-in rate limit). All landed as small focused PRs before the actual flip. "Ready to flip" should not mean "stop reviewing" — the pre-flip window is the cheapest time to harden because the audience is still zero. (From Task 5.2.)
