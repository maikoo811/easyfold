...
- ~~**Container crash-loop detection**~~ — **closed (post-3.3 follow-up).** `dispatch.poll_prediction` now inspects `FunctionCall.get_call_graph()` before falling through to `get(timeout=0)`; `InputStatus.INIT_FAILURE` / `TERMINATED` / `TIMEOUT` flip the job to `"failed"` immediately with an actionable error message ("check `modal app logs <app-name>`"). Previously the route reported `"running"` until the Function's 30-minute timeout fired even for containers crash-looping on a missing image dep. Detection is fail-open: a call-graph API hiccup falls through to `get()` rather than spuriously failing the job.
