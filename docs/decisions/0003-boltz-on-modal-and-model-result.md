...
- **`--no_kernels` quality trade-off.** First real end-to-end run (Task 3.3 validation, 2026-05-24) revealed Boltz's optimized triangular-attention kernels need `cuequivariance-ops-torch` — NVIDIA's CUDA-native ops package with strict ABI requirements that fought our PyTorch 2.4 / CUDA 12.4 image. We deployed with `--no_kernels` to use Boltz's slow PyTorch path; it works but trades ~2-3× wall-clock time and possibly **prediction quality** (the kernels are part of the trained model graph, not just a perf optimization). A p53 prediction completed in 19s with `--no_kernels` but with broadly low pLDDT — separately consistent with p53 being intrinsically disordered, but worth a comparison run once kernels are restored. Follow-up: pin a CUDA-matched `cuequivariance-ops-torch-cu12` wheel and drop `--no_kernels`.
- **First-real-run discoveries** (Task 3.3 validation, 2026-05-24). Patches applied in the same PR as Task 3.3's smoke verification:
  - `add_local_python_source("easyfold")` must be explicit — Modal only copies the deployed file by default; `from easyfold.X import …` fails in the container without this mount.
  - All easyfold runtime deps (`pydantic`, `httpx`, `numpy`, `pyyaml`) must be in `image.pip_install(...)` even if a model framework (boltz / alphafold3) "should" pull them transitively. Python walks the entire import graph at container start.
  - File rename `inference/boltz.py` → `inference/boltz_app.py` to avoid shadowing the `boltz` PyPI package via Modal's `/root/<basename>.py` layout. Same packaging foot-gun class as ADR 0002's `/modal/` directory rule, at file-name granularity.
  - Boltz uses the YAML input file's stem as its output directory key — we now name the input `{job.name}.yaml` (not `input.yaml`) so the parser's expected `boltz_results_{job_name}/predictions/{job_name}/` layout matches.
  - Boltz writes pLDDT in 0-1 range, not 0-100. Parser scales when `max <= 1.0` to keep the unified `ModelResult` contract at 0-100.
